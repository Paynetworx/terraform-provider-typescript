	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			"package_json": {
				...
				Description: "package.json document content",
				Required:    true,
				ForceNew:    true,
			},
			"triggers":{
			...
			"mounts":{
				...
				Description: "additional files to mount to build docker image",
				Optional:	true,
				ForceNew:   true,
...
func getMount(from interface{}) (to AdditionalMount) {
...
func getMounts(from interface{}) (to []AdditionalMount) {
	mounts := from.([]interface{})
	to = make([]AdditionalMount, len(mounts))
...
func resourceTypescriptNodeModulesCreate(d *schema.ResourceData, meta interface{}) error {
	...
	package_json := d.Get("package_json").(string)
	additional_mounts := d.Get("mounts").([]interface{})
	...
	_, err = io.Copy(dest_file, bytes.NewBufferString(package_json))
	if err != nil {
		return err
	}
	...
	for _, additional_mount := range getMounts(additional_mounts) {
		mounts = append(mounts,mount.Mount{
			Type: mount.TypeBind,
			Source: additional_mount.source,
			Target: additional_mount.target,
		})
	}
